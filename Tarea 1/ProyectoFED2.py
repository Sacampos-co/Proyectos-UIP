...
while True:
    os.system("cls")
    print("Arbol en base a la lista del concierto")
    opc = input("\n1.-Visualizar lista entregada \n2.-Inorden \n3.-Preorden \n4.-Postorden \n5.-Buscar \n6.-Salir \n\nElige una opcion -> ")

    if opc == '1':
        if raiz:
            print("Lista aleatoria entregada del concierto: \n")
            print(ListCon)
    elif opc == '2':
        if raiz:
        ...
        if raiz:
            print("Lista en pre-orden: \n")
            preorden(raiz)
    elif opc == '4':
        if raiz:
            print("Lista en post-orden: \n")
            postorden(raiz)
    elif opc == '5':
        nodo = input("\nIngresa el valor a buscar -> ")
        if nodo.isdigit():
            nodo = int(nodo)
            if raiz.buscar_dato(Nodo, raiz.dato) == None:
                print("\nValor no encontrado...")
            else:
                print("\nValor encontrado -> ", raiz.buscar_dato((Nodo, raiz.dato)), " si existe...")
        else:
